...
#define DEFAULT_EXPANSION_FACTOR 2
    ...
    {
        CC_Deque *deque = (CC_Deque *)conf->mem_calloc(1, sizeof(CC_Deque));

        if (!deque)
            return CC_ERR_ALLOC;

        if (!(deque->buffer = (void **)conf->mem_alloc(conf->capacity * sizeof(void *))))
        {
            conf->mem_free(deque);
            return CC_ERR_ALLOC;
        }
    ...
    enum cc_stat cc_deque_copy_shallow(CC_Deque const *const deque, CC_Deque **out)
    {
        CC_Deque *copy = (CC_Deque *)deque->mem_alloc(sizeof(CC_Deque));

        if (!copy)
            return CC_ERR_ALLOC;

        if (!(copy->buffer = (void **)deque->mem_alloc(deque->capacity * sizeof(void *))))
        {
            deque->mem_free(copy);
            return CC_ERR_ALLOC;
        }
    ...
    enum cc_stat cc_deque_trim_capacity(CC_Deque *deque)
    {
        ...
        void **new_buff = (void **)deque->mem_alloc(sizeof(void *) * new_size);
    ...
    static enum cc_stat expand_capacity(CC_Deque *deque)
    {
        ...
        void **new_buffer = (void **)deque->mem_calloc(new_capacity, sizeof(void *));
